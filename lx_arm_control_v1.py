...
class arm_control(object):
    def __init__(self):
        ...
        self.default_pose_ready_using_screw_driver = [0.642, -0.936, 0.767, -1.46, -0.928, 1.68, -3.14/4-0.2,
                                                 -0.477, +0.925, +0.55, +1.550, -1.166, -1.749, -0.704+3.14-0.1,
                                                 0.46, 0, -0.6]
        self.default_jps={
    ...
    def set_gripper(self, value):
        ...
        self.dual_gripper_client.send_goal(goal)
        self.dual_gripper_client.wait_for_result()

        # self.done_thr_set_grippers = True

    def reset(self,mode="insert"):
        assert mode in ["insert","screw"]
        args=self.default_jps[mode]
        ...
        if mode =="insert":
            noise=np.array([0,2.5])#list(np. c.uniform(-1,1,[2])+
            noise=np.array([0,4])#list(np. c.uniform(-1,1,[2])+
            self.move_relate("right",noise)
    ...
    def get_force_z(self, arm):
        ...
        if arm == "left":
            return self.cartesianforce_left[0]
        else:
            return self.cartesianforce_right[0]
